...
indoor_img = [i for i in glob.glob(os.path.join(indoor_img_path, '**'), recursive=True) if os.path.splitext(i)[1] == '.JPG']
indoor_label = [i for i in glob.glob(os.path.join(indoor_label_path, '**'), recursive=True) if os.path.splitext(i)[1] == '.json']
outdoor_img = [i for i in glob.glob(os.path.join(outdoor_img_path, '**'), recursive=True) if os.path.splitext(i)[1] == '.JPG']
